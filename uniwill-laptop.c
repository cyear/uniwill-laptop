...
#define EC_ADDR_CPU_PL1_SETTING 0x0783
#define EC_ADDR_CPU_PL2_SETTING 0x0784
#define EC_ADDR_GPU_PL1_SETTING 0x072D
#define EC_ADDR_GPU_PL2_SETTING 0x072E
#define POWER_MAX 115
...
static int uniwill_get_set_ulong(struct wmi_device *wdev,
    ...
    status =
        wmidev_evaluate_method(wdev, 0x0, UNIWILL_GET_SET_ULONG, &in, &out);
...
static bool uniwill_writeable_reg(struct device *dev, unsigned int reg) {
    ...
    case EC_ADDR_CPU_PL1_SETTING:
    case EC_ADDR_CPU_PL2_SETTING:
    case EC_ADDR_GPU_PL1_SETTING:
    case EC_ADDR_GPU_PL2_SETTING:
    case EC_ADDR_PWM_1:
    case EC_ADDR_PWM_2:
        return true;
    default:
...
static umode_t uniwill_is_visible(const void *drvdata,
...
static int uniwill_read(struct device *dev, enum hwmon_sensor_types type,
    ...
    case hwmon_power:
        switch (channel) {
        case 0: // CPU PL1
            ret = regmap_read(data->regmap, EC_ADDR_CPU_PL1_SETTING, &value);
            break;
        case 1: // CPU PL2
            ret = regmap_read(data->regmap, EC_ADDR_CPU_PL2_SETTING, &value);
            break;
        case 2: // GPU PL1
            ret = regmap_read(data->regmap, EC_ADDR_GPU_PL1_SETTING, &value);
            // sizeof(value));
            break;
        case 3: // GPU PL2
            ret = regmap_read(data->regmap, EC_ADDR_GPU_PL2_SETTING, &value);
            break;
        default:
            return -EOPNOTSUPP;
        }
        ...
        if (ret < 0)
            return ret;
        *val = value * 1000;
        // *val = (value & 0xFF); // cpu_to_be16(value);
        pr_info("*val: %ld\n", *val);
        return 0;
    case hwmon_fan:
        switch (channel) {
...
static int uniwill_read_string(struct device *dev, enum hwmon_sensor_types type,
...
static int uniwill_write(struct device *dev, enum hwmon_sensor_types type,
                         u32 attr, int channel, long val) {
    struct uniwill_data *data = dev_get_drvdata(dev);
    unsigned int value;
    ...
    switch (type) {
    case hwmon_power:
        pr_info("write value: %ld\n", val);
        value = clamp_val(val / 1000, 0, POWER_MAX);
        pr_info("id: %u / write value: %u\n", channel, value);
        switch (channel) {
        case 0:
            return regmap_write(data->regmap, EC_ADDR_CPU_PL1_SETTING,
                                value); // CPU PL1
        case 1:
            return regmap_write(data->regmap, EC_ADDR_CPU_PL2_SETTING,
                                value); // CPU PL2
        case 2:
            return regmap_write(data->regmap, EC_ADDR_GPU_PL1_SETTING,
                                value); // GPU PL1
        case 3:
            return regmap_write(data->regmap, EC_ADDR_GPU_PL2_SETTING,
                                value); // GPU PL2
        default:
            return -EOPNOTSUPP;
        }
...
static const struct hwmon_channel_info *const uniwill_info[] = {
    ...
    HWMON_CHANNEL_INFO(power,
                       HWMON_P_INPUT | HWMON_P_LABEL,  // CPU PL1
                       HWMON_P_INPUT | HWMON_P_LABEL,  // CPU PL2
                       HWMON_P_INPUT | HWMON_P_LABEL,  // GPU PL1
                       HWMON_P_INPUT | HWMON_P_LABEL), // GPU PL2
...
static int uniwill_hwmon_init(struct uniwill_data *data) {
    ...
    hdev = devm_hwmon_device_register_with_info(&data->wdev->dev, "uniwill",
                                                data, &uniwill_chip_info, NULL);
...
static int uniwill_platform_profile_get(struct platform_profile_handler *pprof,
    ...
    struct uniwill_data *data =
        container_of(pprof, struct uniwill_data, profile_handler);
...
static int uniwill_platform_profile_set(struct platform_profile_handler *pprof,
    ...
    return regmap_update_bits(data->regmap, EC_ADDR_MANUAL_FAN_CTRL, mask,
                              value);
}
...
static int
devm_platform_profile_register(struct device *dev,
...
static int uniwill_platform_profile_init(struct uniwill_data *data) {
    ...
    ret = devm_platform_profile_register(&data->wdev->dev,
                                         &data->profile_handler);
    if (ret < 0)
        return ret;
...
static int uniwill_probe(struct wmi_device *wdev, const void *context) {
    ...
    regmap =
        devm_regmap_init(&wdev->dev, &uniwill_ec_bus, data, &uniwill_ec_config);
    if (IS_ERR(regmap))
 ...
 */
static const struct wmi_device_id uniwill_id_table[] = {{UNIWILL_GUID, NULL},
                                                        {}};
